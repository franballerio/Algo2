...
public class SistemaSIU {
    ...
    public void inscribir(String estudiante, String carrera, String materia){
        // primero buscamos estudiante y sumamos a su contador de materias
        trieAlumnos.buscarAlumno(estudiante).materiasInscriptas++;      // como las LU son acotadas toda operacion es: O(1)

        // ahora metemos esta LU en la lista de la materia
        trieDeCarreras.buscarCarrera(carrera).materiasDeCarrera.buscarMateria(materia).materia.agregarAlumno(estudiante);
        // accedemos al ultimo nodo de carrera, de ahi buscamos el ultimo nodo de la materia que apunta al objeto 
        //  de la materia, y en ese objeto enchufamos la LU
        // complejidad es O(largo carrera) + O(largo materia) 
    }

    public void agregarDocente(CargoDocente cargo, String carrera, String materia){
        // if (cargo == PROF) {
        //     trieDeCarreras.buscarCarrera(carrera).materiasDeCarrera.buscarMateria(materia).materia.agregarProfe();
        // } 
    ...
    public int materiasInscriptas(String estudiante){
        return this.trieAlumnos.buscarAlumno(estudiante).materiasInscriptas ; // hay que implementar buscarAlumno en el trieAlumnos	    
    }
